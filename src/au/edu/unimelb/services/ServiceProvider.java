...
//                    break;
                case OPTF:
                    File logDir = new File(fargs[0]);
                    if (!logDir.isDirectory()) {
                        System.out.println("[ERROR] " + fargs[0] + " is not a directory.");
                        return;
                    }

                    File[] logFiles = logDir.listFiles((dir, name) -> name.endsWith(".xes"));
                    if (logFiles == null || logFiles.length == 0) {
                        System.out.println("[INFO] No XES log files found in: " + fargs[0]);
                        return;
                    }

                    for (File logFile : logFiles) {
                        System.out.println("[RUNNING] OPTF on log: " + logFile.getName());
                        try {
                            testProvider.APDO(logFile.getAbsolutePath(), fargs[1], fargs[2], fargs[3]);
                        } catch (Exception e) {
                            System.out.println("[ERROR] Failed on log: " + logFile.getName());
                            e.printStackTrace();
                        }
                    }
                    break;
